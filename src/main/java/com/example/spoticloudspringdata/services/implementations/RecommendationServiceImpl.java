...
public class RecommendationServiceImpl implements RecommendationService {
    ...
    @Autowired
    public RecommendationServiceImpl(UserRepository userRepository, TrackRepository trackRepository) {
    ...
    private String generatePlaylistDescription(List<Track> tracks, User user) {
        List<String> genreNames = tracks.stream()
                .flatMap(track -> track.getTags().stream()
                        .map(Genre::getName)).distinct().limit(3).toList();
        List<String> artistNames = tracks.stream().
                map(track -> track.getArtist().getName()).distinct().limit(3).toList();
